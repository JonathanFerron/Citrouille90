...
### 4.7 MCC Code Generation Sequence

1. Bare metal blink including device clock at 24mhz
   1b. Matrix scanning only (UART debug)
   1c. Encoder only (UART debug)
2. PWM LED pulse
3. USB Vendor (Control EP0), including enabling global interrupts
4. USB HID Boot Keyboard
   4b. USB HID Boot Keyboard + Encoder
5. USB HID full kb: boot Keyboard + extended controls
6. USB HID Boot Mouse + full Keyboard
7. USB HID Composite Full (all 3 interfaces)
   7b.. USB Composite + Matrix + Encoder (3x3 minimal)
8. Full feature set

### 4.8 Integrate Num Lock and Caps Lock led indicators
